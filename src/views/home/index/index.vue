    ...
    <div v-for="(card, index) in store.getters.getNewWebsite" :key="index">
      <WebSites>
        ...
        <template #websiteContent>
          <el-row :gutter="24">
            <el-col v-for="item in card.children" :key="item.id" :md="6" :lg="6" :xl="6">
              <a :href="item.link" style="display: block;text-decoration: none;color:#409EFF;">
                <el-card style="margin-bottom:20px;">
                  <span style="font-size: 32px">
                    <el-image :src="'/path/index/websites/img/'+item.icon" style="width:32px;height:32px"></el-image>
                  </span>
                  <span style="font-size: 1.25rem;font-weight: 530;vertical-align: super;margin-left: 6px;">
                    {{ item.title }}
                  </span>
                  <div><small style="color:#606266;">{{ item.description }}</small></div>
                </el-card>
              </a>
            </el-col>
          </el-row>
